  ...
  document.onkeydown = function (e) {
    if (e.code === 'Enter') {
      nextTick(() => {
        ...
        const inputEles = proxy
          .$(form[formIndex])
          .find(
            '.ant-input,.ant-select-selection-search-input,.ant-picker .ant-picker-input input,.ant-radio-input,.ant-checkbox-input,.ant-input-number-input'
          )
          .not('.ant-input-disabled,.ant-picker-disabled .ant-picker-input input,textarea.ant-input');
        // 过滤出非隐藏输入框
